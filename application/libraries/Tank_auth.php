...
class Tank_auth
{
...

	{
		if ($this->ci->config->item('login_count_attempts', 'tank_auth')) {
			if (!$this->is_max_login_attempts_exceeded($login)) {
				...
				if (!is_null($user_id)) {
					$this->ban_user($user_id,'Por varios intentos fallidos.');
				}
			}
		}
	}
	...
	/**
	 * Bloquear usuario usando un id dado
	 *
	 * @param	string
	 * @param	string
	 * @return	bool
	...
	function ban_user($user_id, $reason)
	{
		...
		if ((strlen($user_id) > 0) AND (strlen($reason) > 0)) {
			return $this->ci->users->ban_user($user_id, $reason);
		}
		return FALSE;
	}

	/**
	 * Desbloquer usuario usando un id dado
	 *
	 * @param	string
	 * @param	string
	 * @param	bool
	...
	function unban_user($user_id)
	{
		...
		if (strlen($user_id) > 0) {
			return $this->ci->users->unban_user($user_id);
		}
		return FALSE;
	}
}
